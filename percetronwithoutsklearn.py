    ...
    def train(self, features, labels):
        ...
        for label in labels:
            for feature in features:
                posmissclassify=list(repeat(0, n+1))
                negmissclassify=list(repeat(0, n+1))
                b=np.append(feature,1)
                value = np.dot(b, w)
                if value >= 0:
                    predictlabel=1
                    if not(predictlabel==label):
                        posmissclassify=posmissclassify+b
                
    
                else:
                    predictlabel=0
                    if not(predictlabel==label):
                        negmissclassify=negmissclassify+b
            w=np.add(w,posmissclassify)
            w=np.subtract(w,negmissclassify)
